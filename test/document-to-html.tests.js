...
test("warning is emitted if paragraph style is unrecognized", function () {
  var document = new documents.Document([
    paragraphOfText("Hello.", "Heading1", "Heading 1"),
  ]);
  var converter = new DocumentConverter();
...
test("warning is emitted if run style is unrecognized", function () {
  var run = runOfText("Hello.", {
    styleId: "Heading1Char",
    styleName: "Heading 1 Char",
  });
...
test("empty rows are preserved in table", function () {
  ...
  return converter.convertToHtml(table).then(function (result) {
    var expectedHtml =
      "<table>" + "<tr><td><p>Row 1</p></td></tr><tr></tr>" + "</table>";
    assert.equal(result.value, expectedHtml);
  });
});
