...
test('style map can be expressed as string', function () {
  ...
  return mammoth
    .convertToHtml({ file: docxFile }, options)
    .then(function (result) {
  ...
  return mammoth
    ...
    .then(function (result) {
      ...
      var warning =
        ...
        'Error was at character number 1: Expected element type but got unrecognizedCharacter "?"';
...
test('src of inline images can be changed using read("base64")', function () {
  ...
  return mammoth
    .convertToHtml({ path: docxPath }, { convertImage: convertImage })
    .then(function (result) {
      assert.deepEqual(result.messages, []);
