...
function DocumentConversion(options, comments) {
    ...
    function convertParagraph(element, messages, options) {
        ...
        if (element.alignment || element.styleId) {
            ...
            if (wp[0] !== undefined) {
                ...
                // var tag = htmlPaths.element('p', { ...wp[0].tag.attributes, ...attr }, { fresh: true });
                // wp[0].tag = tag;
                // **** End *****
        ...
        if (element.attrs && element.children.length > 0) {
            ...
            var hrColor = element.attrs['w:color'] === "auto" ? "#000000" : element.attrs['w:color'];
    ...
    function convertRun(run, messages, options) {
        ...
        // [FS] 09-02-2023
        // Superscript are considered as infoIcon so we dont need this
    ...
    function convertTable(element, messages, options) {
        ...
        if (element.isCusTable) {
            res[0].tag.attributes.style = "border-collapse:collapse;background-color:#d8d8d8;";
        }
        else {
    ...
    var elementConverters = {
        "document": function (document, messages, options) {
            ...
            var footnotesNodes = prepareNote(footNoteHeader, noteReferences, document, messages, options);
            var endnotesNodes = prepareNote(endNoteHeader, noteReferences, document, messages, options);
