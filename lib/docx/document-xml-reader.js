            ...
            if (element.children[i].children[j].name === "w:r") {
                for (var k = 0; k < element.children[i].children[j].children.length; k++) {
                    if (element.children[i].children[j].children[k].name === "w:t") {
                        for (var l = 0; l < element.children[i].children[j].children[k].children.length; l++) {
                           var textVal =element.children[i].children[j].children[k].children[l].value 
                          //      if (textVal.includes() === "Last Updated: " || textVal === "Last Reviewed: "|| textVal==="Last" ||textVal==="Last ") {
                            if (textVal.includes("Last Updated: ")  || textVal.includes("Last Reviewed: ")) {
                                return true;
                            }
                        }
        ...
        if (element.children[i] && element.children[i].name === "w:p") {
            for (var j = 0; j < element.children[i].children.length; j++) {
                if (element.children[i].children[j].name === "w:pPr") {
                    var isBdr = readBottomBorderAttributes(element.children[i].children[j]);
                    ...
                    for (var k = 0; k < element.children[i].children[j].children.length; k++) {
                        if (element.children[i].children[j].children[k].name === "w:pStyle") {
                            if (element.children[i].children[j].children[k].attributes["w:val"] === "LC-ChapHeadLogo") {
    ...
    var Info = Info.children.filter(function (el) {
        for (var i = 0; i < el.children.length; i++) {
            if (el.children[i].name === elName) {
                if (elName == "v:textbox") {
                    if(el.children[i].first("w:txbxContent") && el.children[i].first("w:txbxContent").first("w:p"))
                    {
                        if( el.children[i].first("w:txbxContent").first("w:p").first("w:pPr") && el.children[i].first("w:txbxContent").first("w:p").first("w:pPr").first("w:jc")  )
                        {
var alignElement=el.children[i].first("w:txbxContent").first("w:p").first("w:pPr").first("w:jc")
                            if(alignElement){
...
function replaceParawithTable(element, i, pPr) {
    ...
    if (undefined !== element.children[i] && undefined !== element.children[i].children) {
        for (var l = 0; l < element.children[i].children.length; l++) {
            if (element.children[i].children[l].name === "w:r") {
                if (undefined !== element.children[i] &&
                    undefined !== element.children[i].children[l] &&
                    undefined !== element.children[i].children[l].children) {
                        for (var m = 0; m < element.children[i].children[l].children.length; m++) {
                        if (element.children[i].children[l].children[m] && element.children[i].children[l].children[m].name === "w:pict") {
                            var GroupInfo = null;
                            if (undefined !== element.children[i].children[l].children[m]) {
                                GroupInfo = element.children[i].children[l].children[m].first("v:group");
                            }
                            if (GroupInfo) {
                                ...
                                if (textBoxInfo.length == 0) {
                                    var pictInfo = element.children[i].first("w:r").first("w:pict");
