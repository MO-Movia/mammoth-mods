...
function BodyReader(options) {
    ...
    var xmlElementReaders = {
        ...
        "w:r": function (element) {
            return readXmlElements(element.children)
                .map(function (children) {
                    ...
                    if (children.length <= 1 && children[0] && children[0].type === 'image' && properties) {
        ...
        "v:textbox": function (element) {
            ...
            if (element.attributes.textbox) {
                ...
                txtBoxStyle = element.attributes.textbox.fillcolor ? 'background-color:' + element.attributes.textbox.fillcolor.split(' ')[0] + ';' : '';
                if (element.attributes.textbox.strokeweight) {
                    ...
                    txtBoxStyle = txtBoxStyle + 'border:' + element.attributes.textbox.strokeweight + ' solid ' + element.attributes.textbox.strokecolor.split(' ')[0] + ';';
                ...
                if (txtBoxStyle != '') {
                    ...
                    np.attributes.style = 'border: 1px solid black;display:inline-block;' + style;
        ...
        "v:group": function (element) {
            var gp = readChildElements(element);
            ...
            np.children = gp.value;
            np.attributes.name = 'group';
            var cStyle = readGroupChildrenProps(element);
            // np.attributes.style = element.attributes.style + ';' + cStyle;
            ...
            np.attributes.style = 'display: inline-block;' + nStyle + cStyle;
            // groupStyle = element.attributes.textbox.fillcolor ? 'background-color:' + element.attributes.textbox.fillcolor.split(' ')[0] + ';' : '';
            // if (element.attributes.textbox.strokeweight) {
            //     if (!element.attributes.textbox.strokecolor) {
            //         element.attributes.textbox.strokecolor = 'black';
            //     }
            //     groupStyle = groupStyle + 'border:' + element.attributes.textbox.strokeweight + ' solid ' + element.attributes.textbox.strokecolor.split(' ')[0] + ';';

            // }

            // if (groupStyle != '') {
            //     np.attributes.style = element.attributes.textbox.style.replace('position:absolute;', '') + ';display:block;' + txtBoxStyle;
            // } else {
            //     np.attributes.style = element.attributes.textbox.style.replace('position:absolute;', '') + ';border: 1px solid black;display:block;';
            // }

            gp.value = np;
            gp._result.value.element = np;
    ...
    function readGroupChildrenProps(group) {
        ...
        for (var i = 0; i < group.children.length; i++) {
            if (group.children[i].attributes && group.children[i].attributes.id && group.children[i].attributes.id.includes('AutoShape')) {

                if (!group.children[i].attributes.strokecolor) {
                    group.children[i].attributes.strokecolor = 'black';
                }
                style = 'border:' + group.children[i].attributes.strokeweight + ' solid ' + group.children[i].attributes.strokecolor.split(' ')[0] + ';';
            }
            // [FS] 
            // Vignette Background Color
            if (group.children[i].attributes && group.children[i].attributes.id && group.children[i].attributes.id.includes('Text Box')) {
                if (group.children[i].attributes.fillcolor) {
                    style = style + 'background-color:' + group.children[i].attributes.fillcolor.split(' ')[0] + ';';
                }
            }
    ...
    function readTable(element) {
        ...
        var propertiesResult = readTableProperties(element.firstOrEmpty("w:tblPr"), element.attributes.isCusTbl);
    ...
    function readTableCell(element) {
        return readXmlElements(element.children).map(function (children) {
            ...
            var cell = documents.TableCell(children, { colSpan: colSpan, isLogoImg: properties.attributes.isLogoImg });
    ...
    function readImageData(element) {
        ...
        if (relationshipId) {
            return readImage(
                ...
                element.attributes["o:title"], element.id);
        } else {
